...
//         int type = FileDescriptor[id].second;
//         if (type == READ_ONLY) {
//             cerr << "Unable to write, file is read-only." << endl;
//             return -1;
//         }
...
bool isOpen(char* name) {
    ...
    if (path == NULL)
        return false;
    for (int i = 2; i < MAX_OPEN_FILES; i++) {
        if (FileDescriptor[i] && strcmp(path, FileDescriptor[i]->filePath()) == 0)
            return true;
    }
    return false;
}
...
int SYS_Remove(char* name) {
    if (isOpen(name))
        cerr << "Can't remove open file." << endl;
    else if (kernel->fileSystem->Remove(name)) {
        DEBUG(dbgSys, "Successfully remove " << name);
        return 0;
    }
    DEBUG(dbgSys, "Fail to remove " << name);
